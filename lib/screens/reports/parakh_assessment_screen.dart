      ...
      body: StreamBuilder<List<AssessmentModel>>(
          ...
          builder: (context, snapshot) {
            ...
            if (assessments.isEmpty) {
              return const Center(
                child: Text(
                    'No assessment data found. Please seed the database from the National Dashboard.',
                    textAlign: TextAlign.center),
              );
            }

            // Calculate NIPUN Bharat metrics
            final nipunAssessments = assessments
                .where((a) => a.type == AssessmentType.nipunBharat)
                .toList();
            double literacyAvg = 0;
            double numeracyAvg = 0;
            if (nipunAssessments.isNotEmpty) {
              literacyAvg = nipunAssessments
                      .map((a) => (a.scores['literacy'] as num).toDouble())
                      .reduce((a, b) => a + b) /
                  nipunAssessments.length;
              numeracyAvg = nipunAssessments
                      .map((a) => (a.scores['numeracy'] as num).toDouble())
                      .reduce((a, b) => a + b) /
                  nipunAssessments.length;
            }

            // Calculate NAS averages
            final nasAssessments =
                assessments.where((a) => a.type == AssessmentType.nas).toList();
            ...
            double getAvg(int c) {
              ...
              return classScores[c]!.reduce((a, b) => a + b) /
                  classScores[c]!.length;
            }
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildNasBarChart(
                      getAvg(3), getAvg(5), getAvg(8), getAvg(10)),
                ],
              ),
            );
          }),
